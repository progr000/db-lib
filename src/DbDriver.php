...
class DbDriver
{
    ...
    {
        if (!isset(self::$DbInstances[$db_conf_name])) {
            self::$DbInstances[$db_conf_name] = new self($db_conf_name);
        }

        return self::$DbInstances[$db_conf_name];
    }
     ...
     * @throws ConfigException
     */
    private static function set_debug_data($data)
    {
        if (function_exists('set_debug_data')) {
            try {
                set_debug_data('sqlLog', $data);
            } catch (Exception $e) {
