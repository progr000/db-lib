...
class MigrationDriver
{
    ...
    private function init()
    {
        ...
        try {
            self::$main_db = DbDriver::getInstance(config('databases', [])['default-db-connection-name']);
            $list_in_db = self::$main_db->getAll("SELECT " . self::COLUMN_NAME . " as m FROM {{" . self::TABLE_LIST_MIGRATIONS . "}} ORDER BY migration_name");
            if ($list_in_db) {
    ...
    public function up($steps = null)
    {
        if (!sizeof($this->undone_list)) {
            ...
            //self::$main_db->beginTransaction(); // CREATE, ALTER, DROP can't use with transaction in MySQL
            $cnt = 0;
            foreach ($this->undone_list as $v) {
                if ($steps === null || $cnt < $steps) {
                    ...
                    if ($this->execute($v, 'up') !== false) {
                        self::$main_db->exec("INSERT INTO {{" . self::TABLE_LIST_MIGRATIONS . "}} (" . self::COLUMN_NAME . ") VALUES (:name)", ['name' => $v]);
                        $executeMigration->showSuccess();
                    } else {
                        self::$main_db->rollBack();
                        $executeMigration->showError();
                        //LogDriver::warning(self::$main_db->getErrors(), 0);
            ...
            while (null !== ($v = array_pop($this->done_list))) {
                if ($cnt < $steps) {
                    ...
                    if ($this->execute($v, 'down') !== false) {
                        self::$main_db->exec("DELETE FROM {{" . self::TABLE_LIST_MIGRATIONS . "}} WHERE " . self::COLUMN_NAME . " =  :name", ['name' => $v]);
                        $executeMigration->showSuccess();
                    } else {
                        self::$main_db->rollBack();
                        $executeMigration->showError();
